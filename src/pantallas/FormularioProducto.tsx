...
import { EstadoProducto, Producto, RootStackParamList } from '../tipos/producto';

type FormularioProductoProps =
NativeStackScreenProps<RootStackParamList,'FormularioProducto'>;

const ESTADO_INICIAL: Omit<Producto,
'id_producto'> = {
    nombre: '',
    descripcion: '',
    precio: '',
...
const FormularioProducto: React.FC<FormularioProductoProps> =
 ({ navigation }) => {
    ...
    const handleInputChange = (campo: keyof typeof ESTADO_INICIAL, 
    ...
    const seleccionarFotografia = async () => {
        ...
        if (status !== 'granted') {
            Alert.alert('Permiso Denegated compa', 'NEJECITO acceso a la galería para seleccionar una imagen.');
            return;
        }
        ...
        if (!resultado.canceled) {
            handleInputChange('url_fotografia', resultado.assets[0].uri);
        }
    };

    const handleGuardar = async () => {
        if (!producto.nombre || !producto.precio || !producto.estado) {
        ...
        try {
            ...
            Alert.alert('Éxito', `Producto ${producto.nombre} creado correctamente.`);

            setProducto(ESTADO_INICIAL);
            navigation.navigate('ListaProductos');

        } catch (error) {
            const mensaje = error instanceof Error ? error.message : 'Error desconocido al guardar.';
        ...
        <ScrollView style={estilos.contenedor}>
            ...
            >
                {producto.url_fotografia ? (
                    <Image source={{ uri: producto.url_fotografia }} style={estilos.imagen} />
                ) : (
